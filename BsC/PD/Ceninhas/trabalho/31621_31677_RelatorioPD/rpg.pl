...
%lights
item(19,'Candle', 0).
item(20,'Small Lantern',0).
item(21, 'Icarus Head - Light from the Gods',0).
...
itemQuarto(12, 20, 'Small Lantern').
...
itemQuarto(24, 21, 'Icarus Head - Light from the Gods').
...
jogar(go(N)) :-
	...
	( passagem(X, N, _)
		-> retractall(atual(_)),
			...
			quarto(N, Z, _),
			write('You advanced to '), write(Z), 
			nl,nl
		; write('You cannot go there'), nl, nl
	).
...
jogar(n) :- 
	...
	(passagem(X, Y, n)
		-> quarto(Y, Z, _),
			retract(atual(_)),
			asserta(atual(Y)),
			write('You advanced to '), write(Z), nl,
			recordVar(R),
			( R == y
				-> assertz(record(n))
				; write('')
			), nl
		; write('There is no Exit that way!'), nl, nl
	).
...
jogar(up) :-
	...
	(passagem(X, Y, up)
		-> quarto(Y, _Z, _),
			...
			quarto(X, _, T),
			T1 is T-1,
			write('You went up a floor! You are now on floor: '), write(T1), nl
		; write('There is no way you can go up in here!'), nl
	),
	recordVar(R),
	( R == y
	-> assertz(record(up))
...
jogar(down) :-
	...
	(passagem(X, Y, down)
		-> 	passable(X, Y, P),
			( P == y
				-> (retract(atual(_)), asserta(atual(Y)), quarto(X, _, Z), 
					Z1 is Z+1, write('You went down a floor! You are now on floor: '), 
					write(Z1), nl)
				; (write('There is a monster blocking your way...'), nl)
			)
		; write('There is no way you can go down in here!'), nl	
	),
	recordVar(R),
	( R == y
	-> assertz(record(down))
...
jogar(inv) :- 
	nl,
	(inventario(_K,_L)
	-> forall(inventario(X, Y), (write('Item Number: '), 
		write(X), write(' - '), write(Y), nl))
	; write('You do not have any items on the inventory'), nl
	),
	recordVar(R),
	( R == y
	-> assertz(record(inv))
...
jogar(drop(X)) :-
	...
	(inventario(X, Y)
		...
		; write('You dont have that to drop it!'), nl,nl
	),
	recordVar(R),
	( R == y
...
jogar(get(X)) :-
	...
	(itemQuarto(Y, X, Z)
		-> atual(Y),
			...
			write('You picked up a '), write(Z),
			retract(itemQuarto(Y, X, Z)), nl, !
		; write('You cannot pick that!'), nl
	),
	recordVar(R),
	( R == y
...
jogar(look) :-
	...
	(needLight(X,M)
	-> (inventario(M, _N1)
		->(write('Current Floor: '), write(A), nl, 
			write('Current Room: '), write(N), nl,
			forall(passagem(X, H, Y), ((write('Exit: '), write(Y), 
			write(' (To room number: '), write(H), write(')'), nl))),
			forall(itemQuarto(X, Z, K), ((write('Item number '), 
			write(Z), write(': '), write(K), nl))),
			( monster(ID, _, X)
				-> (write('There is a monster in this room'), nl)
				; write('')
				)
			)
		; write('You need to find some light item on this floor!'), nl
		)
	; (write('Current Floor: '), write(A), nl, 
	write('Current Room: '), write(N), nl,
	forall(passagem(X, H, Y), ((write('Exit: '), write(Y), 
...
jogar(track) :-
	nl,
	write('All steps, since last record.'), nl, nl,
	write('['),
	forall(record(X), ((write(X), write('   ')))), write(']'), nl,nl.


jogar(forget) :-
	nl,
	write('Not recording anymore.'), nl,
	retractall(recordVar(_)),
	asserta(recordVar(n)),
	nl.
...
jogar(warp(X)) :-
	...
	write('The item '), write(X), write(' - '), write(K),
	write(' is now in this Room!'), nl,
	recordVar(R),
	( R == y
...
jogar(destroy(X)) :-
	...
	item(X, K, _Z),
	(itemQuarto(Y, X, K)
		-> write('You have destroyed the item '), write(K),
			retract(itemQuarto(Y, X, K)), nl, !
		; write('The Room doesnt have that item to destroy!'), nl
	),
	recordVar(R),
	( R == y
...
jogar(equip(X)) :-
	...
	(inventario(X, Y)
		-> item(X, Y, _Z),
			...
			( X > 5, X < 11
				-> item(Sword,B,_C),
					asserta(inventario(Sword,B)),
					retract(player(_,_,_)), 
					asserta(player(HP, X, Shield)), retract(inventario(X, Y)),
					write('You equipped the '), write(Y), nl
				; write('')
			),
			( X > 10, X < 16
				-> item(Shield, K, _L),
					asserta(inventario(Shield,K)),
					retract(player(_,_,_)), 
					asserta(player(HP, Sword, X)), retract(inventario(X, Y)),
					write('You equipped the '), write(Y), nl
				; write('')
			),
			( X < 6
				-> write('Cannot equip That!'), nl
				; write('')
			),
			( X > 15
			-> write('Cannot equip That!'), nl
		...
		; write('You dont have that to equip!')
	),
	recordVar(R),
	( R == y
	-> assertz(record(equip(X)))
	; write('')
	),
	nl.
...
jogar(listItem) :-
	nl,
	forall(item(X, Y, _Z), (nl, write('Item number: '), 
	write(X), write('    Name: '), write(Y), nl)),
	recordVar(R),
	( R == y
...
jogar(attack) :-
	...
	( monster(ID, CHP, X)
	-> (monsterID(ID, _, A, D, _), PAttack is PA-D, 
		TotalAttack is PAttack+PetAttack, NHP is CHP-TotalAttack, 
		write('The player attacks the monster for '), write(PAttack), 
		write(' points of damage,'), nl,
		write(' with a little help of '),
		write(PetName), write(' you do an extra '), 
		write(PetAttack), nl, MAttack is A-PD, PNHP is HP-MAttack, 
		write('The monster attacks the player back for '), 
		write(MAttack), write(' points of damage'), nl),
		( PNHP =< 0
			-> (write('You are now dead!, GAME OVER'), nl, break)
			; write('')
		),
		( NHP =< 0 
			-> (write('The monster has been slain!!'), nl, retract(monster(ID,_,_)),
			( ID == c 
				-> write('Congratulations, you won the game!'), nl,
				write('You have done so much exploring today that you feel tired,'), nl,
				write('You then decide to go home...'), nl,
				write('Hey, at least you got epic loot!'), nl, nl, nl

				; retract(passable(X, Y, n)), asserta(passable(X, Y, y))
			)
			)
		; (retract(monster(ID,_,_)), asserta(monster(ID, NHP, X)), 
		retract(cPlayer(_,_)), asserta(cPlayer(PNHP, Pet)))
		)
...
jogar(inspectM) :-
	...
	atual(X),
	( monster(ID, HP, X)
		-> (monsterID(ID, _, A, D, Desc), write('The monsters name is '), 
			write(Desc), nl, write('The monsters attack is '), write(A), nl, 
			write('The monsters defense is '), write(D), nl, write('The monster has '), 
...
jogar(inspectP) :-
	...
	write('On your right hand you have a '), write(N), 
	write(' with '), write(A), write(' attack.'), nl,
	write('On you left hand you have a '), write(N1), 
	write(' with '), write(D), write(' defense.'), nl,
	write('Behind you is '), write(PetName), write(' it is '), 
	write(PetDescription), write('. He adds '), write(PetAttack), write(' attack.'), nl,
	recordVar(R),
	( R == y
			...
			-> (retract(inventario(X, Y)),
				...
				( NHP > 10
					-> retract(cPlayer(_,_)), asserta(cPlayer(10, Pet)),
						...
						write('Your HP is now '), write(10), nl,!
					; retract(cPlayer(_,_)), asserta(cPlayer(NHP, Pet)),
						write('You ate a '), write(Y), nl,
						write('Your HP is now '), write(NHP), nl,!
				) 
			)
		; write('You cannot eat that!'), nl)
	; write('You do not have that to eat!'),nl
	),
	recordVar(R),
	( R == y
	-> assertz(record(eat(X)))
	; write('')
	),
	nl.
...
jogar(summon(X)) :-
	...
	(inventario(X, Y)
		->item(X, Y, _Z),
			cPlayer(HP, _Pet),
			( X = 16
				-> companion(b, PetAttack, PetName, PetDescription), 
					retract(cPlayer(_,_)), asserta(cPlayer(HP, b)), 
					write('You successfully summoned '), write(PetName), nl
				; write('')
			),
			( X = 17
				-> companion(c, PetAttack, PetName, PetDescription), 
					retract(cPlayer(_,_)), asserta(cPlayer(HP, c)), 
					write('You successfully summoned '), write(PetName), nl
				; write('')
			),
			( X = 18
				-> companion(a, PetAttack, PetName, PetDescription), 
					retract(cPlayer(_,_)), asserta(cPlayer(HP, a)), 
					write('You successfully summoned '), write(PetName), nl
				; write('')
			),
			( X < 16
				-> write('You cannot summon that!'), nl
				; write('')
			),
			( X > 18
				-> write('You cannot summon that!'), nl
		...
		; write('You dont have that item to summon!'), nl	
	),
	recordVar(R),
	( R == y
	-> assertz(record(summon(X)))
	; write('')
	),
	nl.
