def main():
    inicio=e_digito(input('Quer jogar? 1-Manualmente 2-Por ficheiro: '))
    if inicio==1:         # chama a funçao correspondente
        vila()
    elif inicio==2:
        por_ficheiro()
    else:                 # se nao for 1 ou 2 o input volta a perguntar 
        main()

def vila():
    # recebe um input ve se e digito e constroi a matriz
    vila=matriz(e_digito(input('inserir a dimensao da aldeia: ')))
    # verifica se a matriz esta dentro dos limites [2,7]
    while len(vila)<2 or len(vila)>7:
        print('A dimensao da sua vila nao esta dominio de [2,7]')
        vila=matriz(e_digito(input('inserir a dimensao da aldeia: ')))
    else:
        # chama a função Habitantes
        habitantes(vila)
        ImprimirVila(vila)
