...
decl *new_decl_fun(decl_kind kind, char *identifier, formal_args *f_args, body *body) {
	decl *new_ = malloc(sizeof(decl));

	new_->kind = kind;
	new_->u1.identifier = strdup(identifier);
	new_->u2.f_args = f_args;
	new_->u3.body = body;

	return new_;
}

id *new_id(char *identifier, kind_t *kind, type_t *type) {
...
formal_args *new_formal_arg(char *identifier, type_t *type, formal_args *f_args) {
 	formal_args *new_ = malloc(sizeof(formal_args));
...
exp *new_exp_compare(exp_kind kind, type_t *type, exp *expression1, exp *expression2, compare_op *compare_op) {
...
exp *new_exp_two(exp_kind kind, type_t *type, exp *expression1, exp *expression2) {
	exp *new_ = malloc(sizeof(exp));
...
exp *new_exp_one(exp_kind kind, type_t *type, exp *expression) {
	exp *new_ = malloc(sizeof(exp));
...
exp *new_exp_id(exp_kind kind, type_t *type, id *id) {
	exp *new_ = malloc(sizeof(exp));
...
exp *new_exp_int_lit(exp_kind kind, type_t *type, int integer) {
	exp *new_ = malloc(sizeof(exp));
...
exp *new_exp_real_lit(exp_kind kind, type_t *type, double real) {
	exp *new_ = malloc(sizeof(exp));
...
exp *new_exp_bool(exp_kind kind, type_t *type, int integer) {
	exp *new_ = malloc(sizeof(exp));
...
exp *new_exp_call(exp_kind kind, type_t *type, fun_call *fun_call) {
	exp *new_ = malloc(sizeof(exp));
...
exp *new_exp_toreal(exp_kind kind, type_t *type, toreal *toreal) {
	exp *new_ = malloc(sizeof(exp));
...
exp *new_exp_empty(exp_kind kind, type_t *type, empty *empty) {
	exp *new_ = malloc(sizeof(exp));
...
type_t *new_type(type_kind kind) {
	type_t *new_ = malloc(sizeof(type_t));
...
kind_t *new_kind(kind_kind kind) {
	kind_t *new_ = malloc(sizeof(kind_t));
