...
%union {
	...
	struct ast_global_decl *global_decl;
...
program : global_decl  														{irGen($1);}													
		;

global_decl : declaration global_decl										{$$ = new_global_decl($1, $2);}
			| declaration 													{$$ = new_global_decl($1, NULL);}
			;
...
formal_args : OPPAR ARG IDENTIFIER type CLPAR formal_args 					{$$ = new_formal_arg($3, $4, $6);}
			| /* empty */													{$$ = new_formal_arg(NULL, NULL, NULL);}
			;
