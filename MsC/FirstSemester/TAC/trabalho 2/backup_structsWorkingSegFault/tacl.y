...
declarations : declaration declarations 													{$$ = new_decls(not_empty, $1, $2);}
		     | /* empty */ 																	{$$ = new_decls(empty_, NULL, NULL);}
		     ;
...
fun_args : OPPAR type IDENTIFIER CLPAR fun_args 											{$$ = new_fargs(not_empty, $2, $3, $5);}
		 | /* empty */ 																		{$$ = new_fargs(empty_, NULL, NULL, NULL);}
		 ;
...
ret : RETURN 																				{$$ = new_ret(return_, NULL);}
	| IRETURN temp  																		{$$ = new_ret(ireturn_, $2);}
	;
...
initial_value : INT_LITERAL 																{$$ = new_ivalue_int(int_value, $1);}
			  | BOOL_LITERAL																{$$ = new_ivalue_bool(bool_value, $1);}
			  ;
